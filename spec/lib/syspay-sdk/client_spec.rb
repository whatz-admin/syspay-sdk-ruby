...
describe SyspaySDK::Client do
  ...
  describe '#generate_auth_header' do
    ...
    it 'returns a properly formatted x-wsse Header' do
      ...
      random_number = 123_456
      timestamp = 789_101_112
      nonce = 'abcdef'
      b64nonce = 'ghijkl'
      digest = 'digested'
  ...
  describe 'Sending a request' do
    ...
    describe '#request' do
      ...
      before do
        ...
        allow(subject).to receive(:build_https_object).and_return https_object
      ...
      it 'retrieves the https object' do
        ...
        expect(subject).to have_received(:build_https_object)
      end
      ...
      describe 'When the request fails' do
        ...
        it 'raises a SyspaySDK::Exceptions::RequestError with the http code and response' do
          begin
            subject.request request_object
          rescue SyspaySDK::Exceptions::RequestError => e
    ...
    describe '#build_https_object' do
      let(:uri)   { double :uri }
      ...
      it { is_expected.not_to respond_to(:build_https_object) }

      it 'creates an URI from syspay_base_url' do
        subject.send :build_https_object
      ...
      it 'returns the http object' do
        expect(subject.send(:build_https_object)).to eq(https)
      end
    end
    ...
    describe '#parse_response' do
      ...
      before do
        ...
        allow(request_object).to receive(:data).and_return data
      end
    ...
    describe '#build_request_for' do
      ...
      before do
        allow(request_object).to receive(:path).and_return path
        allow(request_object).to receive(:data).and_return data
      ...
      describe "When request method is 'GET'" do
        ...
        before do
          allow(request_object).to receive_message_chain(:http_method, :upcase).and_return 'GET'
      ...
      describe "When request method is 'PUT'" do
        ...
        before do
          allow(request_object).to receive_message_chain(:http_method, :upcase).and_return 'PUT'
      ...
      describe "When request method is 'POST'" do
        ...
        before do
          allow(request_object).to receive_message_chain(:http_method, :upcase).and_return 'POST'
      ...
      describe 'When request method is not handled' do
        it 'raises a SyspaySDK::Exceptions::UnhandledMethodError' do
          allow(request_object).to receive_message_chain(:http_method, :upcase).and_return 'DELETE'

          expect do
            subject.send :build_request_for, request_object
          end.to raise_error SyspaySDK::Exceptions::UnhandledMethodError
        end
      end
    end
  end
