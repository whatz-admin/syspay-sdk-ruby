describe SyspaySDK::Requests::PlanUpdate do
  ...
  describe "#get_path" do
    ...
    it "returns the PATH constant with plan_id if exists" do
      ...
      expect(with_plan_id.get_path).to eq("#{described_class::PATH}test_plan_id")
    end
  end
  ...
  describe "#get_data" do
    ...
    describe "the returned hash" do
      it "contains the plan_id" do
        ...
        expect(subject.get_data).to include(plan_id: "plan_id")
      end

      it "contains the trial_amount" do
        subject.trial_amount = "trial_amount"
        expect(subject.get_data).to include(trial_amount: "trial_amount")
      end

      it "contains the initial_amount" do
        subject.initial_amount = "initial_amount"
        expect(subject.get_data).to include(initial_amount: "initial_amount")
      end

      it "contains the billing_amount" do
        subject.billing_amount = "billing_amount"
        expect(subject.get_data).to include(billing_amount: "billing_amount")
      end
    end
  end
end
