  ...
  describe '#data' do
    ...
    describe 'the returned hash' do
      it 'contains a boolean for locked' do
        subject.locked = true
        expect(subject.data).to include(locked: true)

        subject.locked = nil
        expect(subject.data).to include(locked: false)
      end

      it 'contains the type' do
        subject.type = described_class::TYPE_ONESHOT
        expect(subject.data).to include(type: described_class::TYPE_ONESHOT)
      end

      it 'contains the website' do
        subject.website = 'website'
        expect(subject.data).to include(website: 'website')
      end
      ...
      it 'contains the payment_redirect_url' do
        ...
        expect(subject.data).to include(payment_redirect_url: 'payment_redirect_url')
      end

      it 'contains the eterminal_redirect_url' do
        subject.eterminal_redirect_url = 'eterminal_redirect_url'
        expect(subject.data).to include(eterminal_redirect_url: 'eterminal_redirect_url')
      end

      it 'contains the description' do
        subject.description = 'description'
        expect(subject.data).to include(description: 'description')
      end

      it 'contains the reference' do
        subject.reference = 'reference'
        expect(subject.data).to include(reference: 'reference')
      end

      it 'contains the customer' do
        subject.customer = 'customer'
        expect(subject.data).to include(customer: 'customer')
      end

      it 'contains the oneshot' do
        subject.oneshot = 'oneshot'
        expect(subject.data).to include(oneshot: 'oneshot')
      end

      it 'contains the subscription' do
        subject.subscription = 'subscription'
        expect(subject.data).to include(subscription: 'subscription')
      end

      it 'contains the payment_plan' do
        subject.payment_plan = 'payment_plan'
        expect(subject.data).to include(payment_plan: 'payment_plan')
      end

      it 'contains the payment_mandate' do
        subject.payment_mandate = 'payment_mandate'
        expect(subject.data).to include(payment_mandate: 'payment_mandate')
      end

      it 'contains the allowed_retries' do
        subject.allowed_retries = 'allowed_retries'
        expect(subject.data).to include(allowed_retries: 'allowed_retries')
      end
    end
  end
end
