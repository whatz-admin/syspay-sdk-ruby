describe SyspaySDK::Requests::PaymentList do
  ...
  describe 'Constants' do
    ...
    it "has a PATH class constant set to '/api/v1/merchant/payments/'" do
      expect(described_class::PATH).to eq('/api/v1/merchant/payments/')
    end
  end
  ...
  describe 'Initialize' do
    it 'creates a hash of filter' do
      payment_list = described_class.new
      expect(payment_list.filters).to be_a(Hash)
    end
  end
  ...
  describe '#build_response' do
    ...
    it 'returns an Array of SyspaySDK::Entities::Payment' do
      expect(subject.build_response(payments: [])).to be_a(Array)
      expect(subject.build_response(payments: [{ id: 1 }]).first).to be_a(SyspaySDK::Entities::Payment)
    end
  end
end
