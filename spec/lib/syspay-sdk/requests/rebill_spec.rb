describe SyspaySDK::Requests::Rebill do
  ...
  describe "#get_path" do
    ...
    it "returns the PATH constant with billing_agreement_id if exists" do
      ...
      expect(with_billing_agreement_id.get_path).to eq(described_class::PATH.gsub(/:billing_agreement_id/, "test_billing_agreement_id"))
    end
  end
  ...
  describe "#get_data" do
    ...
    describe "the returned hash" do
      it "contains the threatmetrix_session_id" do
        ...
        expect(subject.get_data).to include(threatmetrix_session_id: "threatmetrix_session_id")
      end
      ...
      describe "contains the payment hash which" do
        it "is always present" do
          expect(subject.get_data.keys).to include(:payment)
        end

        it "contains the reference" do
          subject.reference = "reference"
          expect(subject.get_data[:payment]).to include(reference: "reference")
        end

        it "contains the amount" do
          subject.amount = "amount"
          expect(subject.get_data[:payment]).to include(amount: "amount")
        end

        it "contains the currency" do
          subject.currency = "currency"
          expect(subject.get_data[:payment]).to include(currency: "currency")
        end

        it "contains the description" do
          subject.description = "description"
          expect(subject.get_data[:payment]).to include(description: "description")
        end

        it "contains the extra" do
          subject.extra = "extra"
          expect(subject.get_data[:payment]).to include(extra: "extra")
        end
        ...
        it "contains the recipient_map" do
          ...
          expect(subject.get_data[:payment]).to include(recipient_map: [recipient1.to_hash, recipient2.to_hash])
        end
      end
    end
  end
