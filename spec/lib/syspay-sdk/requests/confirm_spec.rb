describe SyspaySDK::Requests::Confirm do
  ...
  describe '#http_method' do
    it 'returns the METHOD constant' do
      expect(subject.http_method).to eq(SyspaySDK::Requests::Confirm::METHOD)
    end
  end
  ...
  describe '#path' do
    it 'returns the PATH constant' do
      expect(subject.path).to eq(SyspaySDK::Requests::Confirm::PATH)
    end
    it 'returns the PATH constant with payment_id if exists' do
      with_payment_id = SyspaySDK::Requests::Confirm.new 'test_payment_id'
      expect(with_payment_id.path).to eq(SyspaySDK::Requests::Confirm::PATH.gsub(/:payment_id/, 'test_payment_id'))
    end
  end
