describe SyspaySDK::Exceptions::RequestError do
  ...
  subject { described_class.new http_code, response_body }
  ...
  it "prints out a specific message" do
    expect(subject.to_s).to eq("The request returned a #{http_code} error with the following body: #{response_body}")
  end
end
